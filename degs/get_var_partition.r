...
aggregate_counts = function( geneExpr, metadata){
  ...
  rownames(md2) = levels(metadata$aggr_unit)
  list( geneCounts  = geneCounts,
        metadata    = md2) 
}

res = aggregate_counts(geneExpr, metadata)
num_nuc = table(metadata$aggr_unit)
res$metadata$nuclei = num_nuc[rownames(res$metadata)]
colnames(res$metadata) = c('subject', 'Biosample', 'clinical', 'COVID', 'ID_10x', 'tissue', 'celltype', 'nuclei')

dge = DGEList( res$geneCounts, samples=res$metadata )
dge = calcNormFactors(dge)

form = ~ (1|tissue) + (1|celltype) + (1|subject)
...
form = ~ (1|tissue) + (1|celltype) + (1|COVID) + (1|subject) + (1|ID_10x) + nuclei
